"""
Minimal YOLOv11 object detection script that draws only bounding boxes.

Usage examples (PyTorch .pt):

- Webcam:   python detect_yolo_simple.py --model yolov11n.pt --source 0
- Video:    python detect_yolo_simple.py --model runs/club_head/yolov11_finetune_cpu/weights/best.pt --source side_reference1.mp4
- Image:    python detect_yolo_simple.py --model yolov11n.pt --source static/example.jpg --save out.jpg

Usage examples (OpenVINO .xml exported by Ultralytics):

- Webcam:   python detect_yolo_simple.py --model runs/club_head/yolov11_finetune_cpu/weights/best_openvino_model/best.xml --source 0
- Video:    python detect_yolo_simple.py --model path/to/best.xml --source side_reference1.mp4 --save out.mp4
- Image:    python detect_yolo_simple.py --model path/to/best.xml --source static/example.jpg --save out.jpg
...
def main():
    ap = argparse.ArgumentParser(description="Simple YOLOv11 detection: draw bounding boxes only")
    ap.add_argument("--model", default="yolov11n.pt", help="Path to YOLOv11 .pt weights")
